...
public class LoginService extends BaseService {
    ...
    public Ret userName(String userName) {
        ...
        if (user == null) {
            user = User.cacheGetByMobile(userName);
        }
        if (user == null) {
            user = User.cacheGetByEmail(userName);
        }
        // 1.用户不存在
        if (null == user) {
        ...
        if (errorCount >= passErrorCount) {
            ...
            float hourSpace = ToolDateTime.getDateMinuteSpace(stopDate, ToolDateTime.getDate())/60f;
            float passErrorHour = PropKit.getFloat(ConstantInit.config_passErrorHour_key);
            if (hourSpace < passErrorHour) { // 密码错误次数超限，几小时内不能登录
                return Ret.create("result", 2)
                        .set("stopDate", Constants.FMT_DATE_TIME.format(user.getStopdate()))
...
//        // 更新缓存
//        User.cacheAdd(user.getPKValue());
        userService.resetAccountStop(user);
    }
