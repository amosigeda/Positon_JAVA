...
public class RedirectRender extends Render {
    public static final Logger LOGGER= LoggerFactory.getLogger(RedirectRender.class);
    private String url;
    ...
    public String buildFinalUrl() {
        ...
        if (contextPath != null && (!url.contains("://") || url.indexOf("://") > 5)) {
        ...
        if (result != null && !result.startsWith("http://") && !result.startsWith("https://")) {
            result = request.getScheme() + "://" + request.getServerName() + (request.getServerPort() == 80 ? "" : ":" + request.getServerPort()) + result;
            LOGGER.debug("\n==========\n{}\n==========\n",request.getScheme());
        }
