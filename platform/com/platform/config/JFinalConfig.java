...
public class JFinalConfig extends com.jfinal.config.JFinalConfig {
    ...
    public static Set<String> noValidPaths = new HashSet<>();
...
//		constants.setLogFactory(new LogBackFactory()); // 参考Log4jLogFactory、Log4jLog实现LogBackFactory、LogBackLog
	...
	public void configPlugin(Plugins plugins) {
		...
		for (String name : dbMap.keySet()) {
			...
			arp.setContainerFactory(new CaseInsensitiveContainerFactory());// 留空区分大小写
        ...
        plugins.add(new SqlMarkDownPlugin());
	...
	public void configInterceptor(Interceptors interceptors) {
        ...
        String noValidPath = PropKit.get(ConstantInit.CONFIG_NOVALIDPATH);
        if (StringUtils.isNotBlank(noValidPath)) {
            noValidPaths.addAll(Arrays.asList(StringUtils.split(noValidPath,",")));
        }
