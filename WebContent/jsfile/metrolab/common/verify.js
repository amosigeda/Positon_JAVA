...
var Verify = function() {
    ...
    var dateTime=function (str) {
        var regex=/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/ig;
        if(regex.test(str)){
            return {"result" : true, "message" : ""};
        }else{
            return {"result" : false, "message" : i18n_utils_msg_format};
	...
	var inputDataVali = function (inputNode){
        ...
        var vMsg = inputNode.attr("vMsg");
		if(null != vType && (type == "text" || type == "password" || type=="hidden")){
			...
			if(value.length != 0){
				if(vType == "email"){//邮箱
				    ...
				    resultArr = date(value);
                }else if(vType == "dateTime"){
				    resultArr = dateTime(value);
                }
    ...
    var showHelpInfo = function (inputNode, info) {
        var helpNode = inputNode.next(".help-inline");
        if (helpNode.length == 0) {
            helpNode = inputNode.parent().next(".help-inline");
        }
        if (helpNode.length == 0) {
            helpNode = $('<span class="help-inline col-sm-2"></span>');
            helpNode.insertAfter(inputNode.parent());
        } else if ($.trim(helpNode.text()) != "" && !inputNode.attr("placeholder")) {
            inputNode.attr("placeholder", $.trim(helpNode.text()));
        }
        helpNode.html(info);
    }

    var hideHelpInfo = function (inputNode) {
        var helpNode = inputNode.next(".help-inline");
        if (helpNode.length == 0) {
            helpNode = inputNode.parent().next(".help-inline");
        }
	 ...
	 * @returns {Number}
	 */
    var formValiSubmit = function (formNode, preVali) {
        var autoSubmit=1;
        if(typeof preVali == "function"){
            if(!preVali()){
                return -1;
            }
        }else{
        ...
        if (errorCount != 0) {
            ...
            return -1;
        } else if (autoSubmit != 2 && typeof common_ajax != "undefined") {
            common_ajax.ajaxFormMainPanel(formNode.id);
        } else if (autoSubmit > 0) {
            $("#" + formNode.id).submit();
        }
        return 1;
    }
...
$(function () {
    $("form").on("blur",":input[vType]",function () {
        Verify.onblurVali(this);
    });
});