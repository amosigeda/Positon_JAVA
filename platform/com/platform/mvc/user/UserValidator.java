...
public class UserValidator extends BaseValidator {
    ...
    protected void validate(Controller controller) {
        ...
        if (actionKey.equals("/platform/user/save")) {
            ...
            // 验证参数
            validateString("password", 6, 256, "passwordMsg", "密码不正确!");
            vali();
            ...
            String userInfoIds = controller.getPara("userInfo.ids");
            if (StringUtils.isNotBlank(userInfoIds) && !"0".equals(userInfoIds)) {
...
//        validateString("user.idcard", 15, 18, "idcardMsg", "身份证号码不正确!");
        validateString("user.names", 2, 15, "namesMsg", "姓名不正确!");
        validateString("user.username", 2, 16, "usernameMsg", "账号不正确!");
    ...
    protected void handleError(Controller controller) {
        String actionKey = getActionKey();
        ((UserController) controller).prepareUserInfo();
        if (actionKey.equals("/platform/user/save")) {
