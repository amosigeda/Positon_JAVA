...
public class Injector {
    ...
    public static <T> List<T> injectModelList(Class<T> modelClass, HttpServletRequest request, boolean skipConvertError) {
        String modelName = modelClass.getSimpleName();
        return injectModelList(modelClass, StrKit.firstCharToLowerCase(modelName), request, skipConvertError);
		}
    ...
    public static <T> List<T> injectModelList(Class<T> modelClass, String modelName, HttpServletRequest request, boolean skipConvertError) {
        Object temp = createInstance(modelClass);
        if (!(temp instanceof Model)) {
            throw new IllegalArgumentException("getModel only support class of Model, using getBean for other class.");
        }
        ...
        for (Entry<String, String[]> entry : parasMap.entrySet()) {
            ...
            try {
                ...
                if (itemList == null) {
                    itemList = new ArrayList<>(paraValueArray.length);
                }
                for (int i = 0; i < paraValueArray.length; i++) {
                    if (i == 0 && itemList.size() == 0) {
                        itemList.add((T) model);
                    } else if (itemList.size() <= i) {
                        model = (Model<?>) createInstance(modelClass);
                        itemList.add((T) model);
                    }
                    String paraValue = paraValueArray[i];
                    Object value = paraValue != null ? TypeConverter.convert(colType, paraValue) : null;
                    ((Model<?>) itemList.get(i)).set(attrName, value);
                }
            } catch (Exception e) {
                if (!skipConvertError) {
                    throw new RuntimeException("Can not convert parameter: " + paraName, e);
