...
public class GroupRoleService extends BaseService {
    ...
    public static void cachRemoveByRoleIds(String roleIds) {
        List<GroupRole> grs = GroupRole.dao.find(getSqlMy("platform.groupRole.findByRoleIds"), roleIds);
        if (grs != null && grs.size() > 0) {
            for (GroupRole gr : grs) {
                cacheRemove(gr.getGroupids());
            }
        }
    }
